...
namespace SecureBoxHack
{
    ...
    public:
        ...
        DynamicBitset(std::size_t size) : bitset(std::ceil(size / static_cast<float>(sizeof(ContainerType) * 8)), 0),
                                          _s(size) {}
        ...
        friend inline std::ostream &helpers::operator<<(std::ostream &os, const DynamicBitset &bs);
